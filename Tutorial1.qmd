...
You'll do code inside of `code chunks` that look like this:
...
Now we see a linear relationship after we take the natural log of GDP per capita. Then we can run an OLS regression like earlier.
...
The results suggest that 1-unit increase in `log(GDP)` is associated with a 2.65 year increase in female life expectancy, and the association is highly statistically significant. However, if we want to interpret the result using the original GDP level, it would be more complicated.
...
A one-unit increase in glucose is associated with a 0.0405 increase in the log-odds of diabetes.

# Why Adding Control Variables is NOT Enough

## Collider Bias

Suppose enrollment in a cardiovascular health program depends on both exercise habits and blood pressure. People who exercise less are more likely to be referred to the program, and people with higher blood pressure are also more likely to be referred. 
...
# More exercise lowers blood pressure
blood_pressure <- 130 -
  0.8 * exercise +
  rnorm(n, mean = 0, sd = 8)

# Enrollment in a cardiovascular health program depends on:
# 1. exercising less
# 2. having higher blood pressure
program_score <-
  -1.5 * as.numeric(scale(exercise)) +
   2.0 * as.numeric(scale(blood_pressure)) +
   rnorm(n, mean = 0, sd = 0.2)

health_program <- ifelse(
  program_score > median(program_score),
  "Enrolled",
  "Not Enrolled"
)
...
In our case, health program enrollment is therefore a collider because it is influenced by both exercise and blood pressure. Let's examine the relationship between exercise and blood pressure with and without controlling for health program.

```{r}
# Model 1: Not controlling collider
no_collider <- lm(blood_pressure ~ exercise, data = data)

# Model 2: controlling collider
collider <- lm(
  blood_pressure ~ exercise + health_program,
  data = data
)

stargazer(no_collider,collider,
          type = "text")
```

In Model 1, exercise is negatively associated with blood pressure: people who exercise more tend to have lower blood pressure. However, after adding health program enrollment as a control in Model 2, the coefficient for exercise becomes positive.

Does this mean that exercising regularly increases blood pressure? Of course not. The misleading result occurs because health program enrollment is a collider. Conditioning on a collider can create a spurious relationship between its causes rather than eliminate bias.
...
Multicollinearity makes individual coefficient estimates unstable and imprecise, leading to large standard errors and difficulty separating the effects of highly correlated predictors. So, be cautious when adding too many highly correlated control variables—sometimes, they do more harm than good!
